...
const ClassCouncil: React.FC = () => {
  ...
  const rows = useMemo(() => {
    ...
    return alunos.map(student => {
      ...
      let mediasAcima5 = 0;
      let mediasAbaixo5 = 0;
      let temPendente = false; // Se falta nota DENTRO do período selecionado
      ...
      disciplinasTurma.forEach(sub => {
        ...
        // Coleta notas apenas até o bimestre de referência
        let sumPoints = 0;
        let countGrades = 0;
        ...
        for (let t = 1; t <= termLimit; t++) {
            ...
            if (g && g.value !== null) {
                ...
                // Se falta nota num bimestre que JÁ deveria ter acontecido (dentro do limite), marca pendente
                // Mas para cálculo de média de "fim de ano abrupto", assumimos 0 ou média parcial?
                // Regra: "Se o ano terminasse hoje". Vamos dividir a soma pelo termLimit.
                // Se o aluno não tem nota, conta como 0 para a média, mas avisa que está pendente.
                missingGradeInPeriod = true;
            }
        }

        // Média é calculada dividindo a soma pelo número de bimestres Decorridos (termLimit)
        // Ex: Se estamos no 2º Bimestre, divide por 2. Se o aluno só fez o 1º, a nota do 2º entra como 0 na média.
        const mg = sumPoints / termLimit;
        
        let mf = mg;
        let isRecovered = false;
        
        // Recuperação Final só se aplica se estivermos analisando o ano todo (4º Bimestre)
        if (termLimit === 4) {
        ...
        // Status "Em Curso" agora significa "Falta nota dentro do período analisado"
        const isCompleteInPeriod = !missingGradeInPeriod;
        const status: 'Aprovado' | 'Retido' | 'Em Curso' = mf >= 6 ? 'Aprovado' : (isCompleteInPeriod ? 'Retido' : 'Em Curso');

        if (mf >= 5) {
          mediasAcima5++;
        } else if (isCompleteInPeriod) {
          mediasAbaixo5++;
          reprovadoTotal = true;
          retidas.push(sub.name);
        }

        if (!isCompleteInPeriod) {
          temPendente = true;
