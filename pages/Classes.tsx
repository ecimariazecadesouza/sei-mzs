...
import { Pencil, Trash2, Users, Book, Copy, Plus, GraduationCap, ChevronRight, BarChart3 } from 'lucide-react';
...
const Icons = {
  ...
  Plus: () => <Plus size={20} />,
  SchoolCap: () => <GraduationCap size={24} />,
...
const Classes: React.FC = () => {
  ...
  const [viewingPerformance, setViewingPerformance] = useState<Class | null>(null);
  ...
  return (
    <div className="space-y-10 pb-20">
        ...
        {filteredClasses.map(cls => {
          ...
          return (
            <div key={cls.id} className="bg-white rounded-[32px] shadow-sm border border-slate-100 p-8 flex flex-col transition-all hover:shadow-xl group relative overflow-hidden">
              ...
              <button onClick={() => setViewingPerformance(cls)} className="flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-100 hover:bg-slate-50 transition-all mb-8 group/btn shadow-sm relative z-10 w-full">
                <div className="flex items-center gap-3 text-slate-700 font-bold text-sm">
                  <Icons.Chart />
      ...
      {/* Modal Performance Chart - Distribuição Detalhada por Disciplina */}
      {viewingPerformance && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[110] p-4">
          <div className="bg-white rounded-[40px] w-full max-w-4xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
            <div className="p-10 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center shrink-0">
              <div>
                <h3 className="text-2xl font-black text-slate-800 tracking-tight">Desempenho da Turma</h3>
                <p className="text-xs font-black text-indigo-600 uppercase tracking-widest mt-1">{viewingPerformance.name} • Distribuição de Notas por Disciplina</p>
              </div>
              <button onClick={() => setViewingPerformance(null)} className="w-12 h-12 bg-white border border-slate-200 rounded-2xl flex items-center justify-center text-slate-400 shadow-sm transition-all hover:text-slate-800">✕</button>
            </div>
              ...
              {(() => {
                ...
                if (classStudents.length === 0 || subjects.length === 0) {
                  return (
                    <div className="text-center opacity-30 py-20">
                      <p className="font-black text-slate-400 uppercase tracking-widest">Dados insuficientes para análise</p>
                    </div>
                  );
                }
                ...
                return subjects.map(sub => {
                  ...
                  classStudents.forEach(std => {
                    const grades = data.grades.filter(g => String(g.studentId) === String(std.id) && String(g.subjectId) === String(sub.id));
                    const bims = [1, 2, 3, 4].map(t => grades.find(g => g.term === t)?.value).filter(v => v !== undefined && v !== null);
                    if (bims.length > 0) {
                      const avg = bims.reduce((a: number, b: any) => a + b, 0) / bims.length;
                      totalGrades++;
                      if (avg < 5) countInsuf++;
                      else if (avg < 6) countReg++;
                      else countOk++;
                    }
                  });
                  ...
                  const pctInsuf = (countInsuf / totalGrades) * 100;
                  const pctReg = (countReg / totalGrades) * 100;
                  const pctOk = (countOk / totalGrades) * 100;

                  return (
                    <div key={sub.id} className="bg-slate-50/50 rounded-3xl p-6 border border-slate-100">
                      <div className="flex justify-between items-end mb-4">
                        <h4 className="font-black text-slate-700 uppercase tracking-tight text-sm">{sub.name}</h4>
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{totalGrades} Médias Calculadas</span>
                      </div>

                      <div className="h-4 w-full bg-white rounded-full overflow-hidden flex shadow-inner">
                        {pctInsuf > 0 && <div style={{ width: `${pctInsuf}%` }} className="h-full bg-red-500" title={`Insuficiente: ${Math.round(pctInsuf)}%`}></div>}
                        {pctReg > 0 && <div style={{ width: `${pctReg}%` }} className="h-full bg-amber-500" title={`Regular: ${Math.round(pctReg)}%`}></div>}
                        {pctOk > 0 && <div style={{ width: `${pctOk}%` }} className="h-full bg-emerald-500" title={`Na Média: ${Math.round(pctOk)}%`}></div>}
                      </div>

                      <div className="flex justify-between mt-3 text-[10px] font-black uppercase tracking-widest text-slate-400">
                        <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-red-500"></div>Insuficiente ({Math.round(pctInsuf)}%)</div>
                        <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-amber-500"></div>Regular ({Math.round(pctReg)}%)</div>
                        <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-500"></div>Na Média ({Math.round(pctOk)}%)</div>
                      </div>
                    </div>
                  );
                });
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-center shrink-0">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center max-w-lg">
                * A distribuição considera a média simples dos bimestres lançados para cada aluno ativo.
              </p>
            </div>
          </div>
        </div>
